...
export default (md: MarkdownIt, options: TableOptions) => {
  ...
  const table_x_plugin: RuleBlock = (state, startLine: number, endLine: number, silent: boolean) => {
    ...
    const handleNewRow = (rowStart: number) => {
      if (trBegin) {
        // Add new tr
    ...
    const handleColumn = (th: boolean, nextLine: number) => {
    ...
    const handleEmbed = (nextLine: number) => {
    ...
    let r, c, text, range: [start: number, end: number];
